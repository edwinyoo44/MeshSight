...
from schemas.pydantic.NodeSchema import NodeTelemetryDeviceResponse
...
class NodeService:
    ...
    async def telemetry_device(
        ...
        try:
            self.logger.error(f"node_id: {node_id}, start: {start_time}, end: {end_time}")
            response: NodeTelemetryDeviceResponse = NodeTelemetryDeviceResponse(
                ...
                items=await self.nodeTelemetryDeviceRepository.fetch_data_by_time_range(node_id, start_time, end_time),
            )
            return response
